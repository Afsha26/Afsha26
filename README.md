# Hi, I'm Afsha Shaikh 👋

🎓 Computer Engineering Student at MCT RGIT, Mumbai

💻 Software Developer | Machine Learning Enthusiast

🌱 Currently Learning:
- Machine Learning
- Data Analytics

🚀 Interested In:
- Android App Development
- AI & Machine Learning
- Database Systems
- Open Source Contributions

📫 Reach Me:
- LinkedIn: https://linkedin.com/in/shaikh-afsha
- Email: afshaikh210426@gmail.com

## 🚀 Featured Projects

### 📰 Fake News Detection Using BERT
Machine Learning project for detecting misinformation using NLP and BERT.

### 🏫 Educational ERP System
Web-based ERP system for managing academic operations.

### 🌸 Flower Recognition App
Android application using OpenCV and Machine Learning.

### 📱 Resume Builder Android Application
Generate professional resumes using Android and SQLite.

### 📊 HR Employee Retention Analysis
Data analytics project to identify employee retention patterns.


![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Afsha26&show_icons=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Afsha26&layout=compact)

## Skills

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white) ![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)

![Android](https://img.shields.io/badge/Android-3DDC84?style=for-the-badge&logo=android&logoColor=white) ![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)



![](https://komarev.com/ghpvc/?username=Afsha26)
